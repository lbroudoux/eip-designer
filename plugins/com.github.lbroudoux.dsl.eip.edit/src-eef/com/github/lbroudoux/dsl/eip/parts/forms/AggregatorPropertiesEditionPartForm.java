...
public class AggregatorPropertiesEditionPartForm extends SectionPropertiesEditingPart implements IFormPropertiesEditionPart, AggregatorPropertiesEditionPart {
	...
	protected Composite createToChannelsReferencesTable(FormToolkit widgetFactory, Composite parent) {
		this.toChannels = new ReferencesTable(getDescription(EipViewsRepository.Aggregator.Properties.toChannels, EipMessages.AggregatorPropertiesEditionPart_ToChannelsLabel), new ReferencesTableListener	() {
		...
		this.toChannels.setHelpText(propertiesEditionComponent.getHelpContent(EipViewsRepository.Aggregator.Properties.toChannels, EipViewsRepository.FORM_KIND));
		...
		this.toChannels.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (e.item != null && e.item.getData() instanceof EObject) {
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(AggregatorPropertiesEditionPartForm.this, EipViewsRepository.Aggregator.Properties.toChannels, PropertiesEditionEvent.CHANGE, PropertiesEditionEvent.SELECTION_CHANGED, null, e.item.getData()));
				}
			}
		...
		"toChannels", propertiesEditionComponent.getEditingContext().getAdapterFactory(), current.eResource()) {
			...
			public void process(IStructuredSelection selection) {
				for (Iterator<?> iter = selection.iterator(); iter.hasNext();) {
					...
					propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(AggregatorPropertiesEditionPartForm.this, EipViewsRepository.Aggregator.Properties.toChannels,
	...
	protected void moveToChannels(EObject element, int oldIndex, int newIndex) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(AggregatorPropertiesEditionPartForm.this, EipViewsRepository.Aggregator.Properties.toChannels, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.MOVE, element, newIndex));
		toChannels.refresh();
	}
	...
	protected void removeFromToChannels(EObject element) {
		propertiesEditionComponent.firePropertiesChanged(new PropertiesEditionEvent(AggregatorPropertiesEditionPartForm.this, EipViewsRepository.Aggregator.Properties.toChannels, PropertiesEditionEvent.COMMIT, PropertiesEditionEvent.REMOVE, null, element));
		toChannels.refresh();
	}
